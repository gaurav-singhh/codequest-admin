...
function validateStructureFormat(content, errors, warnings) {
  ...
  if (!functionNameMatch) {
    ...
    if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(functionName)) {
      errors.push(
        "Function Name must be a valid identifier (letters, numbers, underscore)"
      );
    }
  }
}
...
function validateTestFiles(testsPath, errors, warnings) {
  ...
  const inputFiles = files.filter(
    (f) => f.startsWith("input") && f.endsWith(".txt")
  );
  ...
  if (inputFiles.length === 0) {
    errors.push(
      "No input test files found (should be input1.txt, input2.txt, etc.)"
    );
  }

  if (outputFiles.length === 0) {
    errors.push(
      "No output test files found (should be output1.txt, output2.txt, etc.)"
    );
  }

  if (inputFiles.length !== outputFiles.length) {
    warnings.push(
      `Mismatch between input files (${inputFiles.length}) and output files (${outputFiles.length})`
    );
  }
  ...
  for (let i = 1; i <= Math.max(inputFiles.length, outputFiles.length); i++) {
    ...
    if (inputFiles.includes(inputFile) && !outputFiles.includes(outputFile)) {
      errors.push(
        `Missing output file: ${outputFile} (corresponding to ${inputFile})`
      );
    }

    if (outputFiles.includes(outputFile) && !inputFiles.includes(inputFile)) {
      errors.push(
        `Missing input file: ${inputFile} (corresponding to ${outputFile})`
      );
    }
  }
  ...
  console.log(
    `✅ Found ${inputFiles.length} input files and ${outputFiles.length} output files`
  );
}
...
function validateProblemMd(problemMdPath, errors, warnings) {
  ...
  if (content.trim().length < 100) {
    warnings.push(
      "Problem.md seems very short - consider adding more detailed explanation"
    );
  }
...
if (require.main === module) {
  ...
  if (!problemPath) {
    ...
    console.error(
      "Example: node validate-problem.js problem-submissions/my-problem"
    );
    process.exit(1);
  }
